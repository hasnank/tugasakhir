...
class Visualizer:
    ...
    def animate_1d_regression_multiple_runs(self,runs,**kwargs):
        weight_history_sample = []
        cost_history_sample = []
        for run in runs:
            # select inds of history to plot
            weight_history = run.weight_histories[0][-1]
            cost_history = run.cost_histories[0][-1]
            weight_history_sample.append(weight_history)
            cost_history_sample.append(cost_history)
        start = 0
        ...
        def animate(k):
            ...
            # pluck out current weights 
            w_best = weight_history_sample[current_ind]
            ...
            if show_history == True:
                ax1.cla()
                ax1.scatter(current_ind+1,cost_history_sample[current_ind],s = 60,color = 'r',edgecolor = 'k',zorder = 3)
                self.plot_cost_history_multimodel(ax1,cost_history_sample,start)
