...
class Setup:
    ...
    def least_squares(self,w,iter):
        ...
        y_p = self.y[:,iter]
        old_rep = 0
        if np.size(self.rep) > 1:
            old_rep = self.rep[:,iter]
        
        # compute cost
        cost = np.sum((old_rep + self.model(x_p,w) - y_p)**2)
    ...
    def softmax(self,w,iter):
        ...
        cost = np.sum(np.log(1 + np.exp(-y_p*(old_rep + self.model(x_p,w)))))
        return cost/float(np.size(y_p))
    ...
    def counting_cost(self,w):
        cost = np.sum((np.sign(self.rep + self.model(self.x,w)) - self.y)**2)
        return 0.25*cost 
