...
class Setup:
    ...
    def choose_cost(self,name,runs,**kwargs):
        # create cost on entire dataset
        funcs = cost_functions.Setup(name,self.x_train,self.y_train,self.feature_transforms,runs)
        self.cost = funcs.cost
        self.model = funcs.model
            
        funcs = cost_functions.Setup(name,self.x_valid,self.y_valid,self.feature_transforms,runs)
        ...
        if name == 'softmax' or name == 'perceptron':
            funcs = cost_functions.Setup('twoclass_counter',self.x_train,self.y_train,self.feature_transforms,runs)
            self.counter = funcs.cost
            
            funcs = cost_functions.Setup('twoclass_counter',self.x_valid,self.y_valid,self.feature_transforms,runs)
            self.valid_counter = funcs.cost
            
        if name == 'multiclass_softmax' or name == 'multiclass_perceptron':
            funcs = cost_functions.Setup('multiclass_counter',self.x_train,self.y_train,self.feature_transforms,runs)
            self.counter = funcs.cost
            
            funcs = cost_functions.Setup('multiclass_counter',self.x_valid,self.y_valid,self.feature_transforms,runs)
            self.valid_counter = funcs.cost
