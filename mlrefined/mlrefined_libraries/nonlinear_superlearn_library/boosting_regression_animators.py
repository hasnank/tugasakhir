...
class Visualizer:
    ...
    def animate_trainval_boosting(self,runs,frames,num_units,**kwargs):
        ...
        def animate(k):
            ...
            if show_history == True:
                ax1.cla()
                self.plot_train_valid_errors(ax1,k,train_errors,valid_errors,labels)
    ...
    # 1d regression demo
    def draw_fit_trainval(self,ax,runs,ind):
        # set plotting limits
        ...
        train_inds = runs[ind].train_inds
        valid_inds = runs[ind].valid_inds
