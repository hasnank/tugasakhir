...
class Visualizer:
    ...
    def animate_runs(self,w_hist1,w_hist2,**kwargs):     
        self.w_hist1 = w_hist1
        self.w_hist2 = w_hist2
        ...
        s = np.linspace(0,1,len(self.w_hist1[:round(len(w_hist1)/2)]))
        s.shape = (len(s),1)
        t = np.ones(len(self.w_hist1[round(len(w_hist1)/2):]))
        t.shape = (len(t),1)
        s = np.vstack((s,t))
        self.colorspec = []
        ...
        #### determine viewing region for contour plot and make
        w0min = min([w[0] for w in w_hist1])[0]
        w0max = max([w[0] for w in w_hist1])[0]
        w0gap = (w0max - w0min)*0.25
        w0min -= w0gap
        w0max += w0gap
        
        w1min = min([w[1] for w in w_hist1])[0]        
        w1max = max([w[1] for w in w_hist1])[0]
        w1gap = (w1max - w1min)*0.25
        w1min -= w1gap
        w1max += w1gap
        self.contour_plot(ax2,w0min,w0max,w1min,w1max,num_contours)
        
        w0min = min([w[0] for w in w_hist2])[0]
        w0max = max([w[0] for w in w_hist2])[0]
        w0gap = (w0max - w0min)*0.25
        w0min -= w0gap
        w0max += w0gap
        
        w1min = min([w[1] for w in w_hist2])[0]       
        w1max = max([w[1] for w in w_hist2])[0]
        w1gap = (w1max - w1min)*0.25
        w1min -= w1gap
        w1max += w1gap
        self.contour_plot(ax3,w0min,w0max,w1min,w1max,num_contours)
        

        # start animation
        num_frames = len(self.w_hist1)
        ...
        def animate(k):
            ...
            y_fit1 = np.tanh(w1[0] + x_fit*w1[1])
            y_fit2 = np.tanh(w2[0] + x_fit*w2[1])

            # scatter data
            self.scatter_pts(ax1)
            
            # plot fit to data
            ax1.plot(x_fit,y_fit1,color = 'm',linewidth = 2) 
            ax1.plot(x_fit,y_fit2,color = 'b',linewidth = 2) 

            ###### make right panel - plot contour and steps ######
            if k == 0:
                ax2.scatter(w1[0],w1[1],s = 90,facecolor = color,edgecolor = 'k',linewidth = 0.5, zorder = 3)
                ax3.scatter(w2[0],w2[1],s = 90,facecolor = color,edgecolor = 'k',linewidth = 0.5, zorder = 3)

            if k > 0 and k < num_frames:
                self.plot_pts_on_contour(ax2,k,color,self.w_hist1)
                self.plot_pts_on_contour(ax3,k,color,self.w_hist2)
    ...
    def plot_pts_on_contour(self,ax,j,color,w_hist):
    ...
    def contour_plot(self,ax,w0min,w0max,w1min,w1max,num_contours):
        #### define input space for function and evaluate ####
        w1 = np.linspace(w0min,w0max,100)
        w2 = np.linspace(w1min,w1max,100)
        ...
        ax.set_ylabel('$w_1$',fontsize = 12,rotation = 0,labelpad = 10)
        ax.set_title(r'$g\left(w_0,w_1\right)$',fontsize = 13)

        ax.axhline(y=0, color='k',zorder = 0,linewidth = 0.5)
        ax.axvline(x=0, color='k',zorder = 0,linewidth = 0.5)
        ax.set_xlim([w0min,w0max])
        ax.set_ylim([w1min,w1max])