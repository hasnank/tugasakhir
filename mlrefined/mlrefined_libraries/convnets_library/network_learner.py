...
class Network:
    ...
    ###### setup architecture ######
    def architecture_settings(self,activation_name,layer_sizes,num_kernels):
    ...
    def predict_training(self,x,w):
        ...
        f = self.architectures.training_architecture(x,w[0],w[1])
    ...
    def predict_testing(self,x,w):     
        # feature trasnsformations
        f,stats = self.architectures.testing_architecture(x,w[0],w[1],self.train_stats)
    ...
    def optimizer_settings(self,alpha,max_its,**kwargs):
        ...
        if 'scale' in kwargs:
            scale = kwargs['scale']
        self.w_init = self.architectures.initializer(self.layer_sizes,self.num_kernels,scale)
