...
from . import convolutional_layer
...
class Setup:
    ...
    def choose_convolutions(self,kernel_sizes,**kwargs):
        ...
        self.conv_transformer = convolutional_layer.Setup(kernel_sizes,**kwargs)
        self.conv_layer = self.conv_transformer.conv_layer
        self.conv_initializer = self.conv_transformer.conv_initializer
    ...
    def choose_features(self,layer_sizes,super_type,**kwargs): 
        ...
        if np.size(self.conv_layer) == 0:
            ...
            layer_sizes.insert(0,self.conv_output_size)
    ...
    def choose_cost(self,name,**kwargs):
        ...
        funcs = cost_functions.Setup(name,self.x,self.y,self.conv_layer,self.feature_transforms,**kwargs)
        self.full_cost = funcs.cost
        self.full_model = funcs.model

        # create training and testing cost functions
        funcs = cost_functions.Setup(name,self.x_train,self.y_train,self.conv_layer,self.feature_transforms,**kwargs)
        self.train_cost = funcs.cost
        self.train_model = funcs.model
        
        funcs = cost_functions.Setup(name,self.x_val,self.y_val,self.conv_layer,self.feature_transforms,**kwargs)
    ...
    def fit(self,**kwargs):
        ...
        weight_history,train_cost_history,val_cost_history = optimizers.gradient_descent(self.train_cost,self.val_cost,self.alpha_choice,self.max_its,self.w_init,self.train_num,self.val_num,self.batch_size)
